...
\title{Transformers struggle to use their emergent world models -- a case study on the Towers of Hanoi}
...
\begin{abstract}
%The Towers of Hanoi is a simple planning puzzle, involving N rings and M pegs, that is simple to describe but challenging for large reasoning models. Prior work from 2025 (``the Illusion of Thinking paper'') has shown that the performance of frontier models collapses when N grows to 5 or 6 (M is set to 3), in the most well-known variant of the puzzle where all rings are stacked on one peg and need to be moved to another peg. This failure has never been explained mechanistically.
...
This paper presents an in-depth study of how both small, in-house
Transformers and large, third-party LRMs solve this %e Towers of Hanoi
task. %We start with showing that some state-of-the-art LLMs from 2026 do solve the tower-to-tower variant, motivating us to focus on the `flat-to-flat' variant of the puzzle instead, which remains challenging for the models (without tool use).  
%
To understand the
failures mechanistically, we first train small Transformers from
scratch on precomputed solution traces. Using a variety of
interpretability techniques, we show that these Transformers develop
an emergent world model: a linearly decodable, geometrically faithful
representation of the puzzle's state space (the Sierpi\'nski
triangle), that is causally involved in solving the puzzles. 
%
Second, we return to the large LLMs and apply our techniques to two frontier reasoning models,
Qwen3.6-27B and DeepSeek-R1-Distill-Qwen-32B, that attempt to solve
the task through extended chain-of-thought.  Surprisingly, we find
that both models encode the Sierpi\'nski world model near-perfectly at
the end of the prompt, and yet fail at the majority of tasks when
$N\geq4$. We locate the source of this failure in the decaying
representation of the world model. We probe for the representation at
different stages during planning. We establish causality by showing the performance can be improved either by injecting the prompt-time representation at inference, or by fine-tuning using a state-encoding
auxiliary loss. The results reframe the reported collapse in performance from prior work: these
models build a world model and then lose it. The failure is one of
maintenance, not absence, and it is at least partially recoverable.
\end{abstract}
...
Current large reasoning models are capable of drafting mathematical
proofs, writing complex code, and scoring at the frontier of graduate
examinations. And yet, they cannot -- at least without tool use --
reliably solve a puzzle popularly used to assess children's planning
abilities. \citet{shojaee2025illusion} studied the abilities of such
models on the Towers of Hanoi task, where a stack of rings of
different sizes needs to be moved from the source peg to the goal peg,
while respecting the constraint that larger pegs may not be on top of
smaller pegs and all rings are either on the source or target peg or
on a third, spare peg. Shojaee et al. report that the accuracy of
these models falls off a cliff once the puzzle grows past a handful of
disks, and, stranger still, their reasoning traces get shorter at
exactly the point where the problem gets harder. The paper named this
phenomenon the \enquote{illusion of thinking}, kicking off an intense
debate about the generalizability of the progress on reasoning \cite{varela_rethinking_2025,meyerson_solving_2025,zhao_is_2026}. Despite many follow-ups, the phenomenon has not been explained mechanistically: we know
that these models fail, but we do not know what inside the model is
failing.

In this paper, we present an in-depth case-study into the mechanisms by which both small Transformers and large LRMs (try to) solve the Tower of Hanoi task. We start by describing two variants of the task and our baseline results. we then, in section~\ref{sec:toy} study our in-house trained Transformers, asking whether they show evidence of having acquired an `emergent world model' \citep{nanda2023emergent}. In section~\ref{sec:lrm} we return to LRMs and show evidence that they built up the same emergent world models for this task, but lose it when generating the actual plan.
...
\begin{tikzpicture}[scale=0.25]
  \draw[fill=brown!60!black] (1.5,-0.2) rectangle (10.5,0.2);  % Base
  \foreach \x in {2, 6, 10} {\draw[fill=gray!50!black] (\x-0.1,0) rectangle (\x+0.1,4);} % Pegs
  \node at (2,-1.0) {1}; \node at (6,-1.0) {2}; \node at (10,-1.0) {3}; % Peg Labels
  % Disks (Width, X-center, Y-level)
  \draw[fill=red!70!black, rounded corners=2pt] (1.5,2.6) rectangle (2.5,3.1);   % Disk 1 (Small) on Peg A
  \draw[fill=blue!70!black, rounded corners=2pt] (1.0,1.9) rectangle (3.0,2.4);  % Disk 2 (Medium) on Peg A
  \draw[fill=green!70!black, rounded corners=2pt] (0.5,1.2) rectangle (3.5,1.7);% Disk 3 (Large) on Peg C
  \draw[fill=black, rounded corners=2pt] (0,0.5) rectangle (4,1.0);% Disk 4 (Largest) on Peg B
...
\begin{tikzpicture}[scale=0.25]
  ...
  \node at (2,-1.0) {1}; \node at (6,-1.0) {2}; \node at (10,-1.0) {3}; % Peg Labels
  % Disks (Width, X-center, Y-level)
  \draw[fill=red!70!black, rounded corners=2pt] (9.5,2.6) rectangle (10.5,3.1);   % Disk 1 (Small) on Peg A
  \draw[fill=blue!70!black, rounded corners=2pt] (9.0,1.9) rectangle (11.0,2.4);  % Disk 2 (Medium) on Peg A
  \draw[fill=green!70!black, rounded corners=2pt] (8.5,1.2) rectangle (11.5,1.7);% Disk 3 (Large) on Peg C
  \draw[fill=black, rounded corners=2pt] (8.0,0.5) rectangle (12,1.0);% Disk 4 (Largest) on Peg B
\end{tikzpicture} 
\\
\multicolumn{3}{c}{``tower-to-tower''}
\\\\\\
\begin{tikzpicture}[scale=0.25]
  \draw[fill=brown!60!black] (1.5,-0.2) rectangle (10.5,0.2);  % Base
  \foreach \x in {2, 6, 10} {\draw[fill=gray!50!black] (\x-0.1,0) rectangle (\x+0.1,4);} % Pegs
  \node at (2,-1.0) {1}; \node at (6,-1.0) {2}; \node at (10,-1.0) {3}; % Peg Labels
  % Disks (Width, X-center, Y-level)
  \draw[fill=red!70!black, rounded corners=2pt] (1.5,1.2) rectangle (2.5,1.7);   % Disk 1 (Small) on Peg A
  \draw[fill=blue!70!black, rounded corners=2pt] (1.0,0.5) rectangle (3.0,1.0);  % Disk 2 (Medium) on Peg A
  \draw[fill=green!70!black, rounded corners=2pt] (8.5,0.5) rectangle (11.5,1.0);% Disk 3 (Large) on Peg C
  \draw[fill=black, rounded corners=2pt] (4,0.5) rectangle (8,1.0);% Disk 4 (Largest) on Peg B
\end{tikzpicture} & $\longrightarrow$ &
 \begin{tikzpicture}[scale=0.25]
  \draw[fill=brown!60!black] (1.5,-0.2) rectangle (10.5,0.2);  % Base
  \foreach \x in {2, 6, 10} {\draw[fill=gray!50!black] (\x-0.1,0) rectangle (\x+0.1,4);} % Pegs
  \node at (2,-1.0) {1}; \node at (6,-1.0) {2}; \node at (10,-1.0) {3}; % Peg Labels
  % Disks (Width, X-center, Y-level)
  \draw[fill=red!70!black, rounded corners=2pt] (1.5,0.5) rectangle (2.5,1.0);   % Disk 1 (Small) on Peg A
  \draw[fill=blue!70!black, rounded corners=2pt] (9.0,1.9) rectangle (11.0,2.4);  % Disk 2 (Medium) on Peg A
  \draw[fill=green!70!black, rounded corners=2pt] (8.5,1.2) rectangle (11.5,1.7);% Disk 3 (Large) on Peg C
  \draw[fill=black, rounded corners=2pt] (8.0,0.5) rectangle (12,1.0);% Disk 4 (Largest) on Peg B
\end{tikzpicture}\\
\multicolumn{3}{c}{``flat-to-flat''}
\end{tabular}
\end{minipage}
\hspace{-1cm}
\begin{minipage}[b]{.39\textwidth}
\includegraphics[width=1.2\textwidth]{media/images/sierpinski_4disk.png}
\vspace{-3cm}
\end{minipage}
    \caption{The classical instances of the Tower of Hanoi puzzle are tower-to-tower puzzles (topleft). In psychological research and in this paper, also flat-to-flat instances (bottom-left) are used. Initial states, goal states and the path between them can be visualized as path through a state space corresponding the Sierpi\'nski Triangle (right). The examples on the left correspond to a path from the north tip to the south-east tip along the edge; and to a path from a point just southwest of the center (1132) to the point just left from the southeast tip (1333).}
    \label{fig:placeholder}
\end{figure}

\section{The Tower of Hanoi puzzle}
\label{sec:toh}

The Tower of Hanoi is a simple planning puzzle, invented by French mathematician François Édouard Anatole Lucas in 1883, and introduced into AI and cognitive science by Herbert Simon (1975) \nocite{simon_functional_1975}. In the standard set-up, it involves $M=3$ pegs, and $N=4$ rings of different sizes. 
The player can move one ring at a time from one peg to another, subject to the constraints that only the top ring on a stack can be moved and that a larger ring cannot be placed on top of a smaller disk. 
The task of the player is to find a correct (and possibly shortest) sequence of legal moves from an initial configuration to a goal configuration.

The puzzle --in the most well-known version where both initial and goal configurations have all rings on a single peg ("tower-to-tower")-- is a popular programming assignment, as it allows for an elegant recursive implementation where the solution to the problem of moving a tower of $N$ rings (to the goal peg), includes the solution of moving a tower of $N-1$ rings (to the non-goal peg). 
The puzzle --in a variant where initial and goal configurations can also have rings on multiple pegs ("tower-to-flat", "flat-to-tower", "flat-to-flat")-- is also a popular psychological test, where performance of adults and children of various ages is thought to correlate with executive function \citep{welsh_development_2001}.
Finally, in (recreational) mathematics the puzzle is known for its state space: all possible configurations can be arranged in a shape known as the Sierpi\'nsky triangle (Sierpi\'nsky gasket). This triangle is arranged such that all legal moves correspond to transitions between direct neighbours, and that the points of the triangle correspond to configuration with all rings on one peg. 

In current AI, the Tower of Hanoi puzzle played a major role in the `Illusion of Thinking' paper \citep{shojaee2025illusion}, and the ensuing debate. That paper studied the performance of some of the 2025 frontier reasoning models on 4 tasks, including the tower-to-tower ToH task. The paper provides an extensive prompt (system+ user prompt), that we adapt for the current study. The system prompt contains the following elements: (1) general instructions, (2) the rules of the puzzle, (3) formatting instructions, (4) examples. The user prompt then provides: (1) the initial state, (2) the goal state, and (3) a repeat of the rules. The authors report that, despite prompt optimization, performance drops dramatically for $N\geq4$ rings. 

In contrast, we evaluated a range of 2026 frontier models (without tool use), and find that all of them solve standard tower-to-tower tasks near-perfectly across $n\in\{3,4,5\}$ (DeepSeek-R1 24/25, Kimi-K2-Think 23/25, gpt-oss-120b 25/25, Qwen3.6-27B 24/25), confirming the task is saturated. In our experiments, only the distilled DeepSeek-R1-Distill-Qwen-32B failed on most runs (solving just 1/25).

While the standard tower-to-tower task is thus saturated for current reasoning models, the flat-to-flat formulation is far harder (\Cref{tab:flatflat}). Even the strongest models solve only about half of instances optimally, and accuracy falls steeply with disk count. The final two rows re-evaluate the open-weight models under the much tighter output budget that we will use later in this paper. This isolates how much accuracy depends on reasoning length. Restricting Qwen3.6-27B to that budget collapses its optimal-solve rate from $51\%$ to $14\%$:%, despite an unchanged Position-A representation: 
the model that solves half the instances with a mean of $\approx19{,}000$ generated tokens solves a seventh of them when capped. Notably Qwen3.6-27B generates longer traces than DeepSeek-R1-Distill-Qwen-32B on average ($\approx19{,}000$ vs $\approx14{,}000$ tokens) (a fact we return to for the steering asymmetry).

\begin{table}[t]
  \centering
  \caption{Baseline flat-to-flat accuracy (instances solved optimally) on 100 sampled instances per model. \textbf{Max tokens} is the output budget (32k caps the answer only, with provider-side reasoning beyond it; last two rows use a cap of 4096 for $n= \{3, 4$\} and 8192 for $n = 5$).}
  \label{tab:flatflat}
  \begin{tabular}{lccccc}
    \toprule
    \textbf{Model} & \textbf{Max \# tokens} & \textbf{3 disks} & \textbf{4 disks} & \textbf{5 disks} & \textbf{Total} \\
    \midrule
...
%We test three hypotheses, the first two on a toy Transformer trained from scratch and the third on frontier reasoning models. \textbf{H1:} a simple autoregressive Transformer trained on flat-to-flat Tower of Hanoi traces develops a linearly decodable representation of the disk configuration. \textbf{H2:} this representation is causally implicated in the model's solution. \textbf{H3:} of the three competing explanations, the maintenance account (case~2) holds: large reasoning models do form a representation of the disk configuration but fail to maintain it across the chain-of-thought, so intervening to restore it during generation should recover accuracy.
...
multi-dimensional~\citep{csordas2024recurrent, engels2025not}; the unified-versus-factored format we observe is quantified through principal angles between probe subspaces~\citep{bjorck1973numerical}. Decodability does not confirm that models use this representation, therefore we establish causal involvement by activation patching~\citep{vig2020investigating, meng2022locating, wang2022interpretability} and by activation steering~\citep{turner2023steering, subramani2022extracting, hernandez2023inspecting}. The
...
We train a GPT-2-style decoder-only Transformer~\citep{radford2019language} on tokenised flat-to-flat solution traces (generated symbolically) with a cross-entropy loss masked to the move tokens. %The state of all four disks is read out at the separator token (SEP) that sits between the problem and the solution. 
%\paragraph{Experiments} 
All experiments use the $n=4$ Tower of Hanoi, whose 81 valid configurations %form a Sierpi\'nski triangle and 
yield $6{,}480$ ordered $(s_I,s_G)$ pairs with $s_I\neq s_G$, split 80/20 into $5{,}184$ training and $1{,}296$ validation problems. The model has 6 layers, hidden dimension 128, 4 heads and is trained for 50 epochs. It reaches 99.2\% token-level and 93.2\% sequence-level validation accuracy. 
Solutions are classified \textsc{optimal}, \textsc{suboptimal} (legal, goal-reaching, over-long), \textsc{incorrect} (legal, goal-missing), or \textsc{illegal} (a rule violation or unparseable output).
%We train the toy models to perfection: all outputs are classified as optimal.

\subsection{Distance-matching probe: The joint state is linearly decodable at SEP}

%\paragraph{.} 
To test whether the Sierpi\'nski state space is (linearly) encoded, we train a linear probe $f_\phi:\mathbb{R}^d\!\to\!\mathbb{R}^2$ that maps a hidden state to a 2D embedding whose pairwise Euclidean distances match the graph distances $d_G$ between configurations:
...
\end{equation}
where $\mathcal{S}$ is the set of all 81 four-disk configurations. Probes are trained at each of the 6 layers. We find that over training, the Sierpinski structure becomes more and more apparent figure~\ref{fig:probe-2d}.

%Low loss means the joint state is recoverable through a single linear map. 
...
\begin{figure}[h]
  ...
  \caption{%2D output of the distance-matching probe at layer 5 over training (epochs 5, 25, 50).  
  2D output of the distance-matching probe on the raw residual stream at layer 5, over training (epochs 5, 25, 50). Each point is one of the 81 four-disk configurations, coloured by the largest disk's position; grey edges connect graph-adjacent configurations. The three largest-disk sub-triangles separate into the Sierpi\'nski geometry, with cross-cluster gaps wider than the within-cluster edges. }
  \label{fig:probe-2d}
\end{figure}

Probe results are also reported quantitatively by the Spearman ($\rho$) and Pearson ($r$) correlations between predicted and true pairwise distances, and nearest-state retrieval accuracy (the fraction of configurations whose nearest embedding neighbour is the true nearest graph neighbour). A high-$\rho$ with lower-$r$ signals an order-preserving but non-uniformly scaled embedding. The probe applied to the residual stream at the SEP token recovers the joint four-disk configuration with high fidelity (\Cref{tab:sep-probe}): Spearman correlation reaches $0.94$ at the best layer (layer 5) and Pearson $0.90$, with the geometry assembled by middle depth. The persistent $\rho{-}r$ gap of $3$--$4$ points indicates an embedding that preserves the order of graph distances but inflates the separation between the three sub-triangles defined by the largest disk's position, exactly the recursive cost structure of the puzzle. The converged probe lays the 81 configurations out in the Sierpi\'nski triangle of the state graph (\Cref{fig:sep-probe-epoch50}), a geometry that sharpens over training (\Cref{app:probe-2d}). 

We also train probes on the residual streams at the move-tokens. These probes use the $49{,}450$ move-token activations from optimally solved problems, but, by contrast, fail to recover the joint state (best $\rho=0.78$ with a 15-token window; \Cref{app:move-probes}), even though the same state is perfectly decodable at SEP a few positions earlier, motivating the per-disk analysis below. %This supports \textbf{H1}.

\begin{table}[t]
  \centering
  \caption{Linear probes reveal near-perfect representation of the state space at the SEP's token residual stream. Left: SEP-token distance-matching probe at epoch 50, evaluated on all 81 configurations, by layer. The joint configuration is linearly decodable, peaking at layer 5. Right: Per-disk classification accuracy (best layer, layer 5). Perfect at SEP; the larger disks degrade at move tokens.
 }
  \label{tab:sep-probe}
  \begin{tabular}{rrr}
    \toprule
...
%    \subfig{}
  ...
  \begin{tabular}{l|ll}
    \toprule
    \textbf{Position} & SEP & Move-tokens \\ \midrule
    \textbf{Disk 0} & \textbf{100.0\%} & 99.98\% \\
    \textbf{Disk 1} & \textbf{100.0\%} & 99.96\% \\
    \textbf{Disk 2} & \textbf{100.0\%} & 90.70\% \\ 
    \textbf{Disk 3} & \textbf{100.0\%} & 79.25\% \\ \midrule
    \textbf{Mean} & \textbf{100.0\%} & 92.47\% 
    \bottomrule
  \end{tabular}
\end{table}
...
\subsection{Per-disk probes and subspace analysis: A unified-to-factored shift between planning and execution}
...
\section{Large reasoning models}%: probing through generation, steering, fine-tuning}
\label{sec:lrm}
...
\paragraph{Position A: a world model at the end of the prompt}
\label{sec:lrm-posA}
...
\paragraph{Positions B and C: degradation through generation}
\label{sec:lrm-posBC}
...
\paragraph{Restoring the world model recovers performance}
\label{sec:lrm-steering}

If world model degradation is the bottleneck, restoring the clean Position-A representation at move-emission positions should help. 
%\paragraph{Activation steering.} 
To test causal use we apply activation steering~\citep{turner2023steering}. We cache the layer-$\ell$ Position-A activation $h_{\text{prompt}}(s)$ for each of the 81 configurations and their mean $\bar h$. At each move-emission position, using an externally simulated tracker to recompute the true post-move configuration $s_t$, we add a unit steering vector toward that configuration's clean activation:
...
\paragraph{Restoration in the weights: a state-encoding loss}
...
\bibliography{references,illusion,interpretability}
...
\begin{figure}[h]
  ...
  \begin{minipage}{0.7\textwidth}
    \centering
    \begin{minipage}{0.48\linewidth}\centering Raw residual\\[2pt]\includegraphics[width=\linewidth]{media/images/sae_sep_raw.png}\end{minipage}\hfill
    \begin{minipage}{0.48\linewidth}\centering SAE latent\\[2pt]\includegraphics[width=\linewidth]{media/images/sae_sep_sae.png}\end{minipage}

    \vspace{6pt}
    \begin{minipage}{0.48\linewidth}\centering\includegraphics[width=\linewidth]{media/images/sae_move_raw.png}\end{minipage}\hfill
    \begin{minipage}{0.48\linewidth}\centering\includegraphics[width=\linewidth]{media/images/sae_move_sae.png}\end{minipage}
  \end{minipage}
